...
export default function ProductCard({ product, onLikeChange }) {
  ...
  const handleLike = async (e) => {
    ...
    try {
      const r = await API.post(`/products/${product.id}/like`);
  ...
  return (
    <div className="card" onClick={() => navigate(`/product/${product.id}`)}
      ...
      {/* Photo */}
      <div style={{ position: 'relative', overflow: 'hidden', height: "clamp(220px, 40vw, 280px)", background: 'var(--blush)' }}>
        {imgSrc ? (
      ...
      <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {/* Cart button */}
        <button className="btn-primary" onClick={handleCart} disabled={adding}
          style={{ marginTop: 'auto', width: '100%', justifyContent: 'center', padding: '10px', opacity: adding ? 0.7 : 1 }}>
